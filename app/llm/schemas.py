"""Pydantic models for LLM extraction output — schema v1 + v2."""
...
class ExtractionV2(BaseModel):
    """Compact validation-first schema (default)."""
    ...
    @field_validator("principal_inn", "beneficiary_inn", "bank_inn")
    @classmethod
    def validate_inn(cls, v: str | None) -> str | None:
        if v is None:
            return v
    ...
    @model_validator(mode="after")
    def check_date_order(self) -> ExtractionV2:
        dates = [
            ("issue_date", self.issue_date),
            ("start_date", self.start_date),
            ("end_date", self.end_date),
        ...
        for i in range(len(present) - 1):
            ...
            _n2, d2 = present[i + 1]
            if d1 > d2:
                raise ValueError("Date order is inconsistent: expected issue <= start <= end")
        return self


ValidatedExtraction = ExtractionV1 | ExtractionV2
...
def get_extraction_model(schema_version: str = "v2") -> type[ExtractionV1] | type[ExtractionV2]:
...
def extraction_json_schema(schema_version: str = "v2") -> dict:
    ...
    """
    return get_extraction_model(schema_version).model_json_schema()
